...
Package **nonnest2** was designed to implement Vuong's (1989) theory of non-nested model comparison for many classes of R models.  It has been tested most thoroughly with structural equation models (of class `lavaan`) and with count regression models.  Functionality is available for many other classes, including `lm`, `glm`, `glm.nb`, `clm`, `hurdle`, `zeroinfl`, `mlogit`, `nls`, `polr`, and `rlm`.  Users are cautioned that, while we believe the results to be correct for these models, we have not tested all combinations of models.
